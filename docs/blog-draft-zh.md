...
1. **Shell 腳本** — 十個 bash 腳本涵蓋執行個體探索、相容性 precheck、參數遷移、Blue/Green 部署生命週期、就地升級、升級後驗證和批次協調。所有腳本使用 AWS CLI 進行 RDS 操作，使用 `mysql` 用戶端進行資料庫連線。
...
3. **MCP 伺服器** — 使用 FastMCP 建構的輕量 Python 伺服器，公開八個工具，每個工具包裝一個 shell 腳本。這使 Kiro 能夠透過自然語言命令呼叫這些腳本。

4. **Kiro steering 檔案** — 包含 MySQL 8.0→8.4 升級最佳實務、已知問題和修復模式的知識文件，Kiro 在互動式工作階段中參考此文件。

以下圖表說明解決方案架構。

*[架構圖預留位置 — 使用 AWS Architecture Icons 建立，顯示：DBA → Kiro IDE → MCP Server → Shell Scripts → AWS Cloud (RDS, Blue/Green Deployments, Secrets Manager)]*
...
5. **監控** — 輪詢部署狀態直到綠色環境就緒
6. **Precheck 綠色環境** — 驗證綠色環境通過所有相容性檢查
6b. **Pre-switchover 就緒檢查** — 執行 pre-switchover 就緒檢查，驗證部署狀態、綠色/藍色執行個體可用性、複寫健康狀態、無外部 binlog 複寫，以及版本升級已確認
7. **Switchover** — 執行 Blue/Green switchover 並監控護欄
8. **驗證** — 執行五項升級後健康檢查
9. **清理** — 移除舊的藍色環境
...
- **暫時保留藍色環境。** switchover 後，保留舊的藍色執行個體 24–48 小時。要回復：刪除 B/G 部署（保留舊執行個體），重新命名綠色執行個體，然後將藍色執行個體重新命名回原始名稱。這比快照還原更快。
...
- **謹慎管理並行度。** 對於 Blue/Green 部署，3–5 個並行升級是合理的起點。在批次執行期間監控您帳戶的 RDS 服務配額和 CloudWatch 指標。
...
在整個機群執行 precheck 後，您可能會看到常見模式。隨附的[修復手冊](https://code.amazon.com/packages/Rds-Mysql-Upgrade-Assistant/blobs/main/--/docs/remediation-playbook.md)為每種發現類型提供具體的修復步驟。以下是建議的方法：
...
3. **按影響評估 WARNING 發現。** 並非所有警告都需要採取行動：
   ...
   | 發現 | 是否需要行動？ | 建議 |
   |---------|-----------------|----------------|
   | `sysVarsNewDefaults` | 通常不需要 | 檢閱[預設值變更表](https://code.amazon.com/packages/Rds-Mysql-Upgrade-Assistant/blobs/main/--/docs/remediation-playbook.md)。大多數新預設值是改進。 |
...
有關所有 19 項檢查的完整修復步驟，請參閱[完整修復手冊](https://code.amazon.com/packages/Rds-Mysql-Upgrade-Assistant/blobs/main/--/docs/remediation-playbook.md)。
...
透過自動化升級生命週期，團隊可以減少與主要 MySQL 版本升級相關的時間和風險，將多週的手動工作轉變為可重複、可稽核的程序。shell 優先的架構確保工具可在任何具有 AWS CLI 和 MySQL 用戶端的環境中運作，無需額外基礎設施。
