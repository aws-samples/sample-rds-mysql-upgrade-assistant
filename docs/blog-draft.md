...
- **Manage concurrency carefully.** For Blue/Green deployments, 3–5 concurrent upgrades is a reasonable starting point — the green environment is built independently and production (blue) remains unaffected until switchover. For in-place upgrades, use concurrency of 1 (serial) because each upgrade causes downtime on the target instance; running multiple in-place upgrades simultaneously means multiple databases are unavailable at once. For non-production in-place upgrades where downtime is acceptable, concurrency of 2–3 is fine.
