...
graph TB
    ...
    subgraph Local["Local Environment"]
        direction TB
        Kiro["🔧 Kiro IDE / CLI<br/>(Natural Language)"]
        MCP["MCP Server<br/>FastMCP / stdio<br/>8 tools"]
        Steering["📄 Steering File<br/>RDS-Mysql-Upgrade-Assistant-Guide.md"]

        subgraph Scripts["Shell Scripts (independently runnable)"]
            direction LR
            subgraph Discovery["Discovery"]
                S1["discover_instances.sh"]
                S1b["generate_config.sh"]
            end
            subgraph Precheck["Precheck"]
                S2["mysql_precheck_run.sh<br/>+ phase1.sql (19 checks)"]
            end
            subgraph Params["Params & Options"]
                S3["migrate_param_group.sh"]
                S3b["prepare_param_group.sh"]
                S3c["check_option_group.sh"]
            end
            subgraph Upgrade["Upgrade Lifecycle"]
                S4["create_blue_green.sh"]
                S5["monitor_blue_green.sh"]
                S5b["pre_switchover_check.sh"]
            ...
            subgraph Validate["Validation"]
                ...
                S9b["app_validate_run.sh"]
            end
            subgraph Batch["Orchestration"]
                S10["batch_upgrade.sh"]
            end
            subgraph Lib["Security Libraries"]
                L1["lib/audit_log.sh"]
                L2["lib/integrity_check.sh"]
            end
        end
    end
    ...
    S2 -->|"mysql client TLS"| RDS
    S3 -->|"aws rds describe/modify-db-parameters"| RDS
    S3c -->|"aws rds describe/create-option-group"| RDS
    ...
    S7 -->|"aws rds modify-db-instance/cluster"| RDS
    S8 -->|"aws rds delete-blue-green-deployment"| BG
    S9 -->|"aws rds + mysql client"| RDS
    S2 -.->|"credentials"| SM
    S9 -.->|"credentials"| SM
    S10 --> S1 & S2 & S3 & S3c & S4 & S5 & S5b & S6 & S9 & S9b & S8
...
## Diagram 2: Upgrade Workflow (Blue/Green)

```mermaid
flowchart LR
    subgraph Phase1["Phase 1: Assessment"]
        A["1. Discover"] --> B["2. Precheck<br/>(19 SQL checks)"]
        B --> C["3. Migrate<br/>Params & Options"]
    end

    subgraph Phase2["Phase 2: Blue/Green Upgrade"]
        C --> D["4. Create B/G"]
        D --> E["5. Monitor"]
        E --> F["6. Validate<br/>Green (infra+app)"]
        F --> G["7. Pre-SW<br/>Check"]
        G --> H["8. Switchover"]
    end

    subgraph Phase3["Phase 3: Post-Switchover"]
        H --> I["9. Connectivity<br/>Check"]
        I --> J["10. Cleanup"]
    end

    B -->|"ERROR findings"| SKIP["⛔ SKIP"]
...
## Diagram 3: Upgrade Workflow (In-Place)
...
```mermaid
flowchart TB
    Config["batch_config.yaml<br/>(auto-generated by generate_config.sh)"] --> Parse["Parse Config"]
    Parse --> DedupPG["Dedup Parameter Groups"]
    DedupPG --> DedupOG["Dedup Option Groups"]
    DedupOG --> Queue["Instance Queue"]
