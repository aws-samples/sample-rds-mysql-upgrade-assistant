...
> **摘要：** 当客户拥有 100 个以上需要从 8.0 升级至 8.4 的 RDS MySQL 实例时，他们面临诸多挑战。RDS MySQL Upgrade Assistant 是一个开源工具，提供 19 项 SQL precheck 引擎搭配修复手册、自动化参数和选项组迁移、Blue/Green 和就地升级编排（含 pre-switchover 护栏检查），以及应用程序验证框架——全部可通过 shell 脚本或 Kiro IDE/CLI 的自然语言访问。
...
现有工具解决了部分问题。MySQL Shell 的 `util.checkForServerUpgrade()` 执行兼容性检查，但需要安装 MySQL Shell 并建立连接。RDS 内建的 PrePatchCompatibility 检查只在您实际启动升级时才会执行——如果发生失败，升級作业可能超出维护时段。这两个工具都无法解决批次升级所需的端到端编排。
...
5. **监控** — 轮询部署状态直到绿色环境就绪
6. **验证绿色环境** — 对绿色环境执行基础设施和应用程序验证（`post_upgrade_validate.sh`、`app_validate_run.sh`）
7. **Pre-switchover 检查** — 验证 switchover 护栏：部署状态、复制健康、实例可用性（`pre_switchover_check.sh`）
...
precheck 引擎帮助在執行升级*之前*识别兼容性问题。它以纯 SQL 执行 19 项检查，只需要标准 `mysql` 客户端——不需要安装 MySQL Shell。
...
- **包含 MARIADB_AUDIT_PLUGIN 的自定义选项组** — 创建新的 MySQL 8.4 选项组并添加相同的审计插件。对于 Blue/Green 部署，工具使用两步骤方法：先以相同版本创建 B/G 部署并关联目标选项组，然后单独将绿色实例升级至 8.4（RDS 不支持在单次 B/G 创建中同时指定自定义选项组和主要版本升级）。对于就地升级，目标选项组直接通过 `--option-group-name` 传递。
- **包含 MEMCACHED 的自定义选项组** — MEMCACHED 选项在 MySQL 8.4 中不受支持，会自动排除。如果 MEMCACHED 是唯一的选项，则不创建目标选项组——RDS 使用 `default:mysql-8.4`。
...
-[Kiro IDE](https://kiro.dev/downloads/) 或 [Kiro CLI](https://kiro.dev/docs/cli/installation/)（用于自然语言接口——独立脚本使用不需要）
...
```
# Blue/Green 部署（建议用于生产环境 — 近零停机时间）
"Create a Blue/Green deployment for prod-db-01 upgrading to 8.4.9
 with parameter group prod-mysql84"

# 就地升级（用于非生产环境，或不支持 Blue/Green 的情况）
"Run in-place upgrade for dev-db-01 to 8.4.9 with parameter group dev-mysql84"
```

> **注意：** Kiro 不会自动选择升级策略 — 它执行您指定的操作。对于支持 Blue/Green 的生产实例，建议使用 Blue/Green。对于 Multi-AZ DB Cluster、具有跨区域只读副本的实例或可接受停机的非生产环境，使用就地升级。
...
- **务必先执行 precheck。** 使用 `--dry-run` 模式在执行升级前验证所有实例，並參考[修复手册](https://github.com/aws-samples/sample-rds-mysql-upgrade-assistant/blob/main/docs/remediation-playbook.md)手動修复所有 ERROR 级别的发现。
